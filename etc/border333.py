...
while capture.isOpened():
      ...
      mask2=cv2.inRange(hsv,np.array([70,70,10]),np.array([100,255,255])) # Renk secimi yesil
      ...
      dilation=cv2.dilate(erosion,kernel,iterations=1) #Onceki islem sonucunda elde edilen alani daraltir.
      

      filtered= cv2.GaussianBlur(dilation,(11,11),0)
      ...
      try:
            ...
            for i in contour:
                  ...
                  while(area>30):
                        x[a],y[a],w[a],h[a]=cv2.boundingRect(contours[i-1])
                        cv2.rectangle(crop_image,(x[a],y[a]),(x[a]+w[a],y[a]+h[a]),(0,0,255),3)
                        a=a+1
                        count_defects=a
            '''
            hull=cv2.convexHull(contour)
